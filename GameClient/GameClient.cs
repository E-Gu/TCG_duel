...
{
    //Этот скрипт отвечает за работу клиентской части сетевой карточной игры. Он:
    //Подключается к серверу и отправляет настройки игрока.
    //Отправляет игровые команды (например, разыгрывание карт, атаки, выбор целей).
    //Принимает обновления состояния игры от сервера (новый ход, разыгранная карта, урон и т. д.).
    //Обрабатывает игровые события с использованием Unity Events.
    //Работает как Singleton, чтобы обеспечить доступ к игровому клиенту из любого места кода.
    ...
    public class GameClient : MonoBehaviour
    {
        ...
        {
            int myId = GameClient.Get().GetPlayerID();
            return (playerId == myId) ? "Вы" : "Ваш противник";
        }
