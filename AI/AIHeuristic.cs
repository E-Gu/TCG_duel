    ...
    public class AIHeuristic
    {
        ...
        public int CalculateActionScore(Game data, AIAction order)
        {
            if (order.type == GameAction.EndTurn)
                return -10000; //Other orders are better. Было 0

            if (order.type == GameAction.CancelSelect)
                return -200; //Other orders are better
            ...
            if (order.type == GameAction.Attack)
            {
                ...
                return ascore + oscore + target.attackMax * 5;     //сделала атаку максимум вместо рандомной target.GetAttack()     //Always better to get rid of high-attack cards
            }
            if (order.type == GameAction.AttackPlayer)
            {
                ...
                return ascore + (card.attackMax * 10) - player.hp;    //сделала атаку максимум вместо рандомной target.GetAttack()     //Always better to inflict more damage
            }
            ...
            return 0; // было 100, пробую понизить приоритет завершения хода //Other actions are better than End/Cancel
        }
        ...
        {
            // Если действие EndTurn – возвращаем максимально большое число, чтобы оно сортировалось последним.
            if (order.type == GameAction.EndTurn)
                return int.MaxValue; // EndTurn будет иметь самый высокий sort и выполнится только если нет других вариантов

            if (data.selector != SelectorType.None)
                return 0; // Selector actions не сортируются

            Card card = data.GetCard(order.card_uid);
            Card target = order.target_uid != null ? data.GetCard(order.target_uid) : null;
            bool is_spell = card != null && !card.CardData.IsBoardCard();

            int type_sort = 7; // Базовое значение для остальных действий
                               // Пример приоритетов: для карт, которые не являются спеллами (то есть персонажей), хотим, чтобы они имели приоритет ниже,
                               // а для спеллов – выше. Можно экспериментировать с этими значениями.
            if (order.type == GameAction.PlayCard && is_spell)
                type_sort = 0; // Спеллы – самые высокоприоритетные
            if (order.type == GameAction.CastAbility)
                type_sort = 2; // Способности – вторыми
            if (order.type == GameAction.Move)
                type_sort = 3; // Перемещения – третьими
            if (order.type == GameAction.Attack)
                type_sort = 4; // Атаки – четвёртыми
            if (order.type == GameAction.AttackPlayer)
                type_sort = 5; // Атака по игроку – пятыми
            if (order.type == GameAction.PlayCard && !is_spell)
                type_sort = 1; // Для игровых персонажей – ниже, чем спеллы
