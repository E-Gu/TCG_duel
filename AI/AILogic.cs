...
namespace TcgEngine.AI
{
    ...
    public class AILogic
    {
        ...
        public static AILogic Create(int player_id, int level)
        {
            ...
            {
                // Здесь вместо создания GameLogic создаём DuelLogic.
                // Поскольку на момент создания game данных может еще не быть,
                // можно создать временный объект Game или потом переназначить SetData.
                Game dummyGame = new Game("dummy", 2); // Например, создать dummy объект
                job.game_logic = new DuelLogic(dummyGame);
                //Debug.Log("Создан AI-логика с DuelLogic");
            }
            else
            {
                job.game_logic = new GameLogic(true);
                Debug.Log("Создан AI-логика с GameLogic");
            }
            return job;
        }
        ...
        private void CalculateNode(Game data, NodeState node)
        {
            ...
            if (node.taction < max_actions)
            {
                ...
                {
                    // Добавляем ВСЕ доступные действия
                    for (int c = 0; c < player.cards_hand.Count; c++)
                        AddActions(action_list, data, node, GameAction.PlayCard, player.cards_hand[c]);
            ...
            // 🔥 **НОВЫЙ КОД**: добавляем `EndTurn` ТОЛЬКО если вообще нет других вариантов
            if (action_list.Count == 0)
            {
                Debug.Log("[AI] Нет доступных действий, вынужден выбрать EndTurn.");
                AIAction actiont = CreateAction(GameAction.EndTurn);
                action_list.Add(actiont);
            }

            // Удаляем наихудшие варианты
        ...
        {
            Player player = data.GetPlayer(data.current_player);
            bool duelMode = GameplayData.Get().duel;

            // Если сейчас идёт выбор цели — ничего не делаем.
            if (data.selector != SelectorType.None)
            {
                Debug.Log("[AI] Пропускаем, так как selector != None.");
                return;
            }

            // 🛑 **В дуэльном режиме AI должен сначала играть карту, а не кастовать заклинания.**
            if (duelMode && type != GameAction.PlayCard)
            {
                Debug.Log($"[AI] В дуэли разрешён только PlayCard, {GameAction.GetString(type)} отклонён.");
                return;
            }

            // ✅ **Обрабатываем только PlayCard**
            if (type == GameAction.PlayCard)
            {
                if (card.CardData.IsBoardCard()) // Если карта — персонаж
                {
                    Slot slot = Slot.GetDefaultSlot(player.player_id);
                    Debug.Log($"[AI] Свободные слоты у игрока {player.player_id}: {player.GetEmptySlots().Count}");

                    if (data.CanPlayCard(card, slot))
                    {
                        AIAction action = CreateAction(type, card);
                        action.slot = slot;
                        action.sort = int.MinValue; // Высший приоритет в дуэльном режиме
                        actions.Add(action);
                        Debug.Log($"[AI] Добавлено действие PlayCard для {card.card_id} в слот {slot.x}-{slot.p}");
                    }
                    else
                    {
                        Debug.LogWarning($"[AI] НЕ МОЖЕТ разыграть карту {card.card_id}, слот занят!");
                    }
                }
            }
        }
        ...
        public List<AIAction> GetAvailableActions(Game game_data)
        {
            Player player = game_data.GetPlayer(ai_player_id);

            // Проверяем, есть ли карты у AI
            //Debug.Log("[AI] Проверка карт в руке. Количество карт: " + player.cards_hand.Count);
            foreach (Card card in player.cards_hand)
            {
                //Debug.Log("[AI] В руке есть карта: " + card.card_id);
            }
            List<AIAction> actions = new List<AIAction>();

            // 1️⃣ Проверяем возможность кастовать заклинание
            foreach (Card card in player.cards_hand)
            {
                foreach (AbilityData ability in card.GetAbilities())
                {
                    ...
                    {
                        //Debug.LogWarning("[AI] AI не может использовать способность: " + ability.id + " у карты " + card.card_id);
                    }
                    if (game_data.CanCastAbility(card, ability))
                    {
                        AIAction action = new AIAction(GameAction.CastAbility);
                        action.card_uid = card.uid;
                        action.ability_id = ability.id;
                        actions.Add(action);
                        Debug.Log("[AI] Добавлено доступное действие: CastAbility (" + ability.id + ") для карты " + card.card_id);
                    }
                }
            }

            // 2️⃣ Проверяем возможность выложить карту (с учётом дуэльного режима)
            foreach (Card card in player.cards_hand)
            {
                if (!game_data.CanPlayCard(card, Slot.GetDefaultSlot(player.player_id)))
                    {
                        //Debug.LogWarning("[AI] AI не может играть карту " + card.card_id);
                    }
                if (game_data.CanPlayCard(card, Slot.GetDefaultSlot(player.player_id))) // Проверяем, можно ли играть карту
                {
                    AIAction action = new AIAction(GameAction.PlayCard);
                    action.card_uid = card.uid;
                    action.slot = Slot.GetDefaultSlot(player.player_id); // В дуэльном режиме слот только один
                    actions.Add(action);
                    //Debug.Log("[AI] Добавлено доступное действие: PlayCard (" + card.card_id + ")");
                }
            }

            // 3️⃣ Если действий нет, возвращаем пустой список (EndTurn убираем!)
            if (actions.Count == 0)
            {
                Debug.LogWarning("[AI] Нет доступных действий!");
            }
